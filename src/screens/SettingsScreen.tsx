...
import { createLayoutMotionConfig, motion, getMotionDuration } from '../theme/motion';
      ...
      >
        <Animated.View
          style={[
            styles.toggleThumb,
            {
              backgroundColor: value ? palette.accentOnSolid : palette.bgSurface,
  ...
  <View style={styles.toggleRow}>
    <View style={styles.toggleTextWrap}>
      <View style={styles.toggleTitleLine}>
        <Ionicons name={icon} size={16} color={disabled ? palette.textMuted : palette.accentPrimary} />
        <Text
          variant="body"
          style={[
            styles.toggleTitle,
    ...
    <View style={styles.toggleTrailing}>
      {statusLabel ? (
        <View style={[styles.lockedBadge, { backgroundColor: withAlpha(palette.accentPrimary, 0.12), borderColor: palette.accentBorder }]}>
          <Text variant="bodySmall" style={[styles.lockedBadgeText, { color: palette.accentPrimary }]}>
            {statusLabel}
          </Text>
        </View>
      ) : null}
  ...
  const content = (
    <View style={[styles.actionRow, disabled && styles.actionRowDisabled]}>
      ...
      <View style={styles.actionTextWrap}>
        ...
        {description ? (
          <Text variant="bodySmall" style={[styles.actionDescription, { color: palette.textMuted }]}>
            {description}
          </Text>
        ) : null}
      </View>
      <View style={styles.actionTrailing}>
        {busy ? (
          ...
          <Text variant="bodySmall" style={[styles.actionRightText, { color: palette.textMuted }]}>
            {rightText}
          </Text>
        ) : (
...
export const SettingsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const t = useCallback((key: string) => translateLiteral(key, language), [language]);
  ...
  const restoreBaselineSettings = useCallback(() => {
    ...
    const baselineNotificationTimerMode = baselineNotificationTimerModeRef.current;
  ...
  const handleReplayTour = useCallback(() => {
    ...
    const activeLanguage = languageRef.current;
    const title = translateLiteral('Discard changes and replay tour?', activeLanguage);
    const message = translateLiteral(
    ...
    Alert.alert(title, message, [
      { text: translateLiteral('Keep editing', activeLanguage), style: 'cancel' },
      { text: translateLiteral('Discard', activeLanguage), style: 'destructive', onPress: discardAndReplay },
    ]);
  ...
  const persistSettings = useCallback(async () => {
    ...
    try {
      ...
      await notificationService.setReminderVibrationEnabled(vibrationRef.current);
  ...
  const handleSavePress = useCallback(() => {
    ...
    const message = 'These changes will update how GapWalk looks and behaves. Do you want to save them now?';
    const confirmSave = () => {
  ...
  const handleExportWalkHistory = useCallback(async () => {
    ...
    try {
      ...
      const rows = sessions.map((session) => {
        ...
        const distance = session.distanceMeters != null ? Math.round(session.distanceMeters) : '';
        const calories = session.calories != null ? Math.round(session.calories) : '';
  ...
  const handleOpenPolicy = useCallback((url: string) => {
    closeInfoOverlay();
  ...
  const handleToggleWalkCard = useCallback((card: WalkDisplayCard) => {
    animateSettingChange();
    ...
    if (isOn) {
      if (walkDisplayCards.length <= 2) {
        Alert.alert('Minimum Cards', 'At least 2 cards must stay visible on the walking screen.');
        return;
      }
      setWalkDisplayCards(walkDisplayCards.filter((currentCard) => currentCard !== card));
      return;
    }

    const ordered = ALL_WALK_DISPLAY_CARDS.filter(
      (currentCard) => walkDisplayCards.includes(currentCard) || currentCard === card,
    );
    setWalkDisplayCards(ordered);
  }, [animateSettingChange, setWalkDisplayCards, walkDisplayCards]);

  const setThemeModeWithAnimation = useCallback((nextThemeMode: ThemeMode) => {
    if (nextThemeMode === themeMode) return;
    animateSettingChange();
    setThemeMode(nextThemeMode);
  }, [animateSettingChange, setThemeMode, themeMode]);

  const setLanguageWithAnimation = useCallback((nextLanguage: 'en' | 'es') => {
    if (nextLanguage === language) return;
    animateSettingChange();
    setLanguage(nextLanguage);
  }, [animateSettingChange, language, setLanguage]);

  const setDistanceUnitWithAnimation = useCallback((nextDistanceUnit: 'km' | 'mi') => {
    if (nextDistanceUnit === distanceUnit) return;
    animateSettingChange();
    setDistanceUnit(nextDistanceUnit);
  }, [animateSettingChange, distanceUnit, setDistanceUnit]);

  const setFirstDayWithAnimation = useCallback((nextFirstDay: 'sun' | 'mon') => {
    if (nextFirstDay === firstDayOfWeek) return;
    animateSettingChange();
    setFirstDayOfWeek(nextFirstDay);
  }, [animateSettingChange, firstDayOfWeek, setFirstDayOfWeek]);

  const setVibrationWithAnimation = useCallback((nextValue: boolean) => {
  ...
  const setNotificationTimerWithAnimation = useCallback((nextMode: NotificationTimerMode) => {
  ...
  return (
    <Container scrollable>
      <View style={styles.content}>
        ...
        <SettingsSection title="App Experience" palette={palette}>
          ...
          >
            <AnimatedChoiceGroup
              ...
              options={[
                { label: darkLabel, value: 'dark', testID: 'settings-theme-dark' },
                { label: lightLabel, value: 'light', testID: 'settings-theme-light' },
              ]}
              themeMode={themeMode}
            />
          </SettingShell>
          ...
          >
            <AnimatedChoiceGroup
              ...
              options={[
                { label: englishLabel, value: 'en', testID: 'settings-lang-en' },
                { label: espanolLabel, value: 'es', testID: 'settings-lang-es' },
              ]}
              themeMode={themeMode}
            />
          </SettingShell>
        ...
        <SettingsSection title="Walking Experience" palette={palette}>
          ...
          >
            <AnimatedChoiceGroup
              value={distanceUnit}
              onChange={(next) => setDistanceUnitWithAnimation(next as 'km' | 'mi')}
              options={[
                { label: 'Kilometers', value: 'km', testID: 'settings-unit-km' },
          ...
          <SettingShell
            ...
            infoText={'Smart switches automatically. If you start a walk from a GapWalk reminder, it shows minutes left. Otherwise it shows minutes walked.\n\nMinutes walked always shows how long you have been walking.\n\nMinutes left shows the time remaining toward the current walk target.'}
          ...
          >
            <AnimatedChoiceGroup
              ...
              onChange={(next) => setNotificationTimerWithAnimation(next as NotificationTimerMode)}
              options={[
                { label: NOTIFICATION_TIMER_MODE_LABELS.smart, value: 'smart', testID: 'settings-notification-timer-smart' },
                { label: NOTIFICATION_TIMER_MODE_LABELS.elapsed, value: 'elapsed', testID: 'settings-notification-timer-elapsed' },
                { label: NOTIFICATION_TIMER_MODE_LABELS.remaining, value: 'remaining', testID: 'settings-notification-timer-remaining' },
              ]}
              themeMode={themeMode}
          ...
          <SettingShell
            ...
            onInfoToggle={handleInfoToggle}
          >
            <View style={[styles.innerToggleCard, { backgroundColor: withAlpha(palette.textPrimary, themeMode === 'dark' ? 0.02 : 0.04), borderColor: palette.borderSoft }]}>
              {ALL_WALK_DISPLAY_CARDS.map((card, index) => {
                ...
                return (
                  <React.Fragment key={card}>
                    {index > 0 ? <SectionDivider color={palette.borderSoft} /> : null}
                    <ToggleRow
                      icon={iconName}
        ...
        <SettingsSection title="Weekly & Calendar" palette={palette}>
          ...
          >
            <AnimatedChoiceGroup
              ...
              onChange={(next) => setFirstDayWithAnimation(next as 'sun' | 'mon')}
              options={[
                { label: 'Sunday', value: 'sun', testID: 'settings-week-start-sun' },
                { label: 'Monday', value: 'mon', testID: 'settings-week-start-mon' },
              ]}
              themeMode={themeMode}
            />
          </SettingShell>
        ...
        <SettingsSection title="Reminders & Feedback" palette={palette}>
          ...
          >
            <AnimatedChoiceGroup
              ...
              options={[
                { label: onLabel, value: 'on', testID: 'settings-vibration-on' },
                { label: offLabel, value: 'off', testID: 'settings-vibration-off' },
              ]}
              themeMode={themeMode}
            />
          </SettingShell>
        ...
        {isE2E ? (
          <Card elevated style={styles.e2eCard}>
            <Text variant="bodySmall" style={[styles.sectionLabel, styles.e2eLabel, { color: palette.textMuted }]}>
        ...
        >
          <TwoActionBar
            style={styles.footer}
            noteText={hasUnsavedChanges ? 'Review your changes, then confirm save.' : 'All current settings are saved.'}
            primaryAction={{
              title: t('Save'),
      ...
      <InfoTooltipOverlay activeInfo={activeInfo} onDismiss={closeInfoOverlay} />
      <AppModal
      ...
      >
        <View style={styles.confirmDialogBody}>
          <Text variant="body" style={[styles.confirmDialogMessage, { color: palette.textMuted }]}>
